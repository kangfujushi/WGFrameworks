...
#define HomeFilePath [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0]

//设置发送的文件的最大空间5MB
#define FileMaxSize 10000000
///屏幕高度/宽度
...
+ (instancetype)fileManager:(void(^)(id obj))block {
...
{
    if (_tabvlew == nil) {
        CGRect frame = CGRectMake(0,kStatusBarAndNavigationBarHeight + 5, CJScreenWidth, CJScreenHeight  - toolBarHeight - departmentH - 10);
...
- (KsFileDepartmentView *)departmentView
{
    if (_departmentView == nil) {
        CGRect frame = CGRectMake(0, kNavigationBarHeight, CJScreenWidth, departmentH);
...
- (void)loadData{
    ...
    for(NSString *str in subPathsArray){
        ...
        if ([@[@"doc",@"rar",@"7z",@"xlsx",@"zip",@"xls",@"docx",@"dwg"] containsObject:str.pathExtension.lowercaseString]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.Clickblock = ^(KsFileObjModel *model,UIButton *btn){
        if (weakSelf.selectedItems.count>=1 && btn.selected) {
            btn.selected =  NO;
            model.select = btn.selected;
            [weakSelf.view makeToast:@"最多支持1个文件选择" duration:0.5 position:CSToastPositionCenter];
            return ;
        }
        if ([weakSelf checkFileSize:model]) {
            ...
            [weakSelf.view makeToast:@"暂时不支持超过10MB的文件" duration:0.5 position:CSToastPositionCenter];
            btn.selected =  NO;
            model.select = btn.selected;
        }
    };
...
- (void)didClickSenderButtonInAssetGridToolBar:(KsFileManagerToolBar *)internalAssetGridToolBar
{
    ...
    if (self.block) {
        self.block(self.selectedItems.firstObject);
    }
