 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Description:(NSString *)description Block:(void(^)(void))block
{
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description];
    updateAlert.block = block;
    [[UIApplication sharedApplication].delegate.window addSubview:updateAlert];
}
...
- (void)_setupUI
{
    ...
    //获取更新内容高度
    CGFloat descHeight = [self _sizeofString:self.desc font:[UIFont systemFontOfSize:SELDescriptionFont] maxSize:CGSizeMake(self.frame.size.width - Ratio(80) - Ratio(56+40), 1000)].height;
    ...
    //更新内容
    UITextView *descTextView = [[UITextView alloc]initWithFrame:CGRectMake(Ratio(50), Ratio(10) + CGRectGetMaxY(versionLabel.frame), updateView.frame.size.width - Ratio(56+40), descHeight)];
